 ...
 *
 * Wire format (NestJS backend envelope):
 *
 *   { success: true, data: <T>, meta: { ... } }
 *   { success: false, error: { code, message, details } }
 *
 * The response interceptor auto-detects the envelope and unwraps `data`,
 * so callers receive `T` directly. Non-envelope responses (e.g. raw
 * health checks) are passed through unchanged.
 *
 * NOTE: When the backend is unavailable (e.g. local dev before `pnpm
 * --filter backend dev` is started), feature pages should still render
 * using their mock fallback (see `src/lib/mock/*.ts`). The pages own
 * that fallback — this client just surfaces a network error toast.
 */
...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError<ApiResponse>) => {
    ...
    const apiError: ApiError = {
      ...
      message:
        envelope?.error?.message ?? error.message ?? "An unexpected error occurred",
    ...
    if (status === 403 && typeof window !== "undefined") {
      toast.error("Permission denied", { description: apiError.message });
      return Promise.reject(apiError);
    }

    if (typeof window !== "undefined") {
      if (status === 0) {
        // Network error — quiet (likely local dev, backend not started).
        // Don't toast here; pages handle their own fallbacks.
      } else if (status >= 500) {
