 ...
 * directly. Non-envelope responses (e.g. raw health checks) are passed
 * through unchanged.
 */
...
/**
 * Detect if a response payload is an `ApiResponse` envelope.
 * We require `success` to be a boolean AND either `data` or `error` to
 * be present — this avoids false positives on arbitrary domain objects.
 */
...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // X-Request-Id for tracing (matches backend `RequestIdMiddleware`)
    ...
    // Active tenant header (for SUPER_ADMIN impersonation)
...
apiClient.interceptors.response.use(
  (response) => {
    ...
    // If the response is an envelope, unwrap.
    if (isApiResponseEnvelope(payload)) {
      if (!payload.success) {
        // Backend reported a logical error inside a 2xx — surface as a rejection.
      ...
      // `meta` is preserved on a sidecar property for paginated helpers.
  ...
  (error: AxiosError<ApiResponse>) => {
    ...
    // 401 — token expired or missing; clear + redirect to /login.
    ...
    // 403 — surfaced as a quiet toast (user is logged in but lacks permission).
    ...
    // Network / CORS / timeout — surface as a toast.
    if (typeof window !== "undefined") {
      if (status === 0) {
        ...
        // 422 (validation) and 404 (not found) are typically handled by the form
        // or the page itself — don't double-toast.
...
// ===== API Methods =====
//
// `api.get<T>` returns `Promise<T>` — the unwrapped envelope `data`.
// `api.paginated<T>` returns `Promise<PaginatedResponse<T>>` (data + meta).
...
export const api = {
  ...
  /**
   * Paginated GET — returns both the unwrapped `data` array and the
   * `meta` pagination block.
   *
   * @example
   *   const { data, meta } = await api.paginated<User>("/users", {
  ...
  paginated: async <T>(
    ...
    const meta = (res as unknown as { _meta?: ApiResponse["meta"] })._meta ?? {};
    let data: T[];
    if (Array.isArray(res.data)) {
      ...
      // Some endpoints return `{ data: [...], ... }` without an envelope.
...
//
// The original api.ts bundled per-domain typed API objects
// (`healthApi`, `knowledgeApi`, `voiceApi`, `telephonyApi`,
// `whatsappApi`) plus their snake_case types. Existing dashboard pages
// import these from `@/lib/api` directly. New code should use the
// service layer in `@/services/` instead — these exports exist solely
// to keep existing pages compiling during the migration.
